...
function KpiCard({ label, value, sub, change, color, sparkData, icon, delay }) {
  ...
  return (
    <div
      className="relative overflow-hidden rounded-2xl bg-white border border-white/[0.06] p-5 hover:border-white/[0.12] transition-all duration-300"
    ...
    >
      ...
      <div className="flex items-start justify-between mb-4 relative z-10">
        <div>
          <p className="text-[10px] font-semibold indigo-500/30 uppercase tracking-[0.12em] mb-1.5">{label}</p>
          <p className="text-[26px] font-bold indigo-500 tracking-tight leading-none" style={{ fontFamily: "'DM Mono', monospace" }}>
            {value}
          </p>
          {sub && <p className="text-[11px] indigo-500/35 mt-1">{sub}</p>}
        </div>
      ...
      <div className="relative z-10 flex items-center gap-2">
        ...
        <span className="text-[11px] indigo-500/25">vs last period</span>
      </div>
    </div>
  );
}
...
function SectionTitle({ children, action }) {
  return (
    <div className="flex items-center justify-between mb-4">
      <h2 className="text-[10px] font-semibold indigo-500/30 uppercase tracking-[0.12em]">{children}</h2>
      {action && (
...
export default function Analytics() {
  ...
  return (
    <div className="min-h-screen indigo-500 indigo-500">
      <style>{`
      ...
      {/* ──────────── TOP BAR ──────────── */}
      <header className="sticky top-0 z-50 indigo-500/90 backdrop-blur-xl border-b border-white/[0.06] px-6 py-4">
        <div className="max-w-[1400px] mx-auto flex items-center justify-between gap-4">
          <div>
            <h1 className="text-[15px] font-bold tracking-tight">Analytics</h1>
            <p className="text-[11px] indigo-500/30 mt-0.5">Real-time store performance</p>
          </div>
          ...
          <div className="flex items-center gap-3 flex-wrap justify-end">
            ...
            <div className="flex items-center bg-white/[0.04] rounded-xl p-1 border border-white/[0.06] gap-0.5">
              {["7d", "30d", "90d"].map(r => (
                <button key={r} onClick={() => setRange(r)}
                  className={`px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all ${range === r
                    ? "bg-indigo-600 indigo-500 shadow-lg"
                    : "indigo-500/40 hover:indigo-500/70"
                    }`}>
                  {r}
                </button>
              ))}
            ...
            {/* Export */}
            <button className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-semibold indigo-500/50 border indigo-500 bg-white/[0.03] hover:border-white/[0.12] hover:indigo-500/80 transition-all">
        ...
        <div className="max-w-[1400px] mx-auto flex gap-0">
          {TABS.map(t => (
            <button key={t} onClick={() => setActiveTab(t)}
              className={`px-4 py-3.5 text-xs font-semibold capitalize border-b-2 -mb-px transition-all ${activeTab === t
                ? "border-indigo-500 text-indigo-400"
                : "border-transparent indigo-500/30 hover:indigo-500/60"
                }`}>
              {t}
            </button>
          ))}
      ...
      <main className="max-w-[1400px] mx-auto px-6 py-7 space-y-5">
          ...
          <div className="rounded-2xl bg-white border border-white/[0.06] p-6">
            ...
            <div className="flex justify-between mt-2">
              {(range === "7d"
                ...
                <span key={d} className="text-[10px] indigo-500/20">{d}</span>
              ))}
            </div>
          </div>
            ...
            <div className="flex justify-center mb-5">
              <div className="relative">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                  ...
                  <p className="text-[10px] indigo-500/30 mt-0.5">sessions</p>
                </div>
              </div>
            </div>
            <div className="space-y-3">
              {TRAFFIC_SOURCES.map(s => (
                <div key={s.label} className="flex items-center gap-3">
                  ...
                  <span className="text-xs indigo-500/50 flex-1 min-w-0 truncate">{s.label}</span>
                  <div className="w-20 flex-shrink-0">
                    <div className="h-[3px] rounded-full bg-white/[0.06] overflow-hidden">
                      <div className={`h-full rounded-full ${s.barColor}`} style={{ width: `${s.value}%` }} />
                    </div>
                  </div>
                  <span className="text-xs font-semibold font-mono indigo-500/60 w-8 text-right">{s.value}%</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[1fr_240px_270px] gap-5"
          ...
          {/* Top pages table */}
          <div className="rounded-2xl bg-white border border-white/[0.06] p-6 overflow-x-auto">
            <SectionTitle action="View all →">Top Pages</SectionTitle>
            <table className="w-full min-w-[460px]">
              <thead>
                <tr className="border-b border-white/[0.05]">
                  {["Page", "Views", "Bounce", "Trend"].map(h => (
                    <th key={h} className="pb-3 text-left text-[10px] font-semibold indigo-500/25 uppercase tracking-widest last:text-right">
                      {h}
                    </th>
                  ))}
                </tr>
                  ...
                  <tr key={i} className="hover:bg-white/[0.03] transition-colors cursor-pointer group">
                    <td className="py-3 pr-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm indigo-500/75 group-hover:indigo-500/90 transition-colors truncate max-w-[180px]">
                          {page.path}
                        </span>
                      </div>
                    </td>
                    <td className="py-3 pr-4 text-sm indigo-500/50 font-mono">
                      {page.views.toLocaleString()}
                    </td>
                    <td className="py-3 pr-4 text-sm indigo-500/40">{page.bounce}</td>
              ...
              {GEO_DATA.map((g, i) => (
                <div key={i}>
                  <div className="flex justify-between items-center mb-1.5">
                    <span className="text-xs indigo-500/55">{g.city}</span>
                    <span className="text-[11px] indigo-500/30 font-mono">{g.sessions.toLocaleString()}</span>
                  </div>
                  <div className="h-[3px] rounded-full bg-white/[0.06] overflow-hidden">
              ...
              {RECENT_EVENTS.map((ev, i) => (
                <div key={i}
                  ...
                  <div className="min-w-0">
                    <p className="text-[12px] indigo-500/55 leading-snug">{ev.msg}</p>
                    <p className="text-[10px] indigo-500/25 mt-0.5">{ev.time}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4"
          ...
          {[
            ...
            <div key={d.label}
              className="rounded-2xl bg-white border border-white/[0.06] p-5 flex items-center gap-4 hover:border-white/[0.12] transition-all">
              <Ring pct={d.pct} color={d.color} icon={d.icon} />
              <div>
                <p className={`text-xl font-bold leading-none font-mono ${d.textClass}`}>{d.pct}%</p>
                <p className="text-[11px] indigo-500/30 mt-1">{d.label}</p>
              </div>
            </div>
          ))}

          {/* Bounce rate */}
          <div className="rounded-2xl bg-white border border-white/[0.06] p-5 hover:border-white/[0.12] transition-all">
            <p className="text-[10px] font-semibold indigo-500/25 uppercase tracking-[0.12em] mb-2">Bounce Rate</p>
          ...
          {/* Avg session */}
          <div className="rounded-2xl bg-white border border-white/[0.06] p-5 hover:border-white/[0.12] transition-all">
            <p className="text-[10px] font-semibold indigo-500/25 uppercase tracking-[0.12em] mb-2">Avg Session</p>
