...
const ConditionBadge = ({ value }) => {
  ...
  return (
    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
      isIssue ? 'bg-rose-500/15 text-rose-400' : 'bg-emerald-500/15 text-emerald-400'
    }`}>
      {value?.toUpperCase()}
    </span>
  );
              ...
              {activeTab === 'exterior' && car?.exterior_tyres && (
                <div className="space-y-4">
                  ...
                  <Card>
                    ...
                    <PartRow label="LHS Front" data={car.exterior_tyres.door?.lhs_front} />
                    <PartRow label="LHS Rear" data={car.exterior_tyres.door?.lhs_rear} />
                    <PartRow label="RHS Front" data={car.exterior_tyres.door?.rhs_front} />
                    <PartRow label="RHS Rear" data={car.exterior_tyres.door?.rhs_rear} />
                  </Card>
                  ...
                  <Card>
                    ...
                    {['lhs_a','lhs_b','lhs_c','rhs_a','rhs_b','rhs_c'].map(k => (
                      <PartRow key={k} label={k} data={car.exterior_tyres.pillar?.[k]} />
                    ))}
                  </Card>

                  {/* Fenders */}
                  <Card>
                    <SectionLabel>Fenders & Quarter Panels</SectionLabel>
                    <PartRow label="Fender LHS" data={car.exterior_tyres.fender?.lhs} />
                    <PartRow label="Fender RHS" data={car.exterior_tyres.fender?.rhs} />
                    <PartRow label="Quarter Panel LHS" data={car.exterior_tyres.quarter_panel?.lhs} />
                    <PartRow label="Quarter Panel RHS" data={car.exterior_tyres.quarter_panel?.rhs} />
                  </Card>

                  {/* Glass */}
                  <Card>
                    <SectionLabel>Windshield & Glass</SectionLabel>
                    <PartRow label="Front Windshield" data={car.exterior_tyres.windshield?.front} />
                    <PartRow label="Rear Windshield" data={car.exterior_tyres.windshield?.rear} />
                  </Card>

                  {/* Lights */}
                  <Card>
                    <SectionLabel>Lights</SectionLabel>
                    <PartRow label="LHS Headlight" data={car.exterior_tyres.lights?.lhs_headlight} />
                    <PartRow label="RHS Headlight" data={car.exterior_tyres.lights?.rhs_headlight} />
                    <PartRow label="LHS Taillight" data={car.exterior_tyres.lights?.lhs_taillight} />
                    <PartRow label="RHS Taillight" data={car.exterior_tyres.lights?.rhs_taillight} />
                  </Card>
                  ...
                  <Card>
                    ...
                    {['lhs_front','lhs_rear','rhs_front','rhs_rear','spare'].map(k => (
                      <PartRow key={k} label={k} data={car.exterior_tyres.tyres?.[k]} />
                    ))}
                  </Card>
                      ...
                      <>
                        ...
                        {et.comments && (
                          <Card>
                            ...
                            <p className="text-white/60 text-sm">{et.comments}</p>
                          </Card>
                        )}
                      </>
                    );
...
const Enquiries = () => {
  ...
  const [selectedId, setSelectedId] = useState(null); // only store ID
  ...
  const openAssignModal = (enquiry) => {
    setAssignTarget(enquiry); setSelectedTechId(''); setAssignPriority('medium'); setAssignModalOpen(true);
  };
  const openNoteModal = (enquiry) => {
    setAssignTarget(enquiry); setNoteText(''); setActionType('note'); setActionModalOpen(true);
  };
  const openCostModal = (enquiry) => {
    setAssignTarget(enquiry); setEstimatedCost(enquiry.estimatedCost || ''); setActualCost(enquiry.actualCost || ''); setActionType('cost'); setActionModalOpen(true);
  };
  ...
  const handleAddNote = async () => {
    ...
    try {
      ...
      toast.success('Note added successfully'); setActionModalOpen(false); fetchEnquiries();
  ...
  const handleUpdateCost = async () => {
    try {
      ...
      await axios.put(`${import.meta.env.VITE_API_URL}/api/admin/enquiries/${assignTarget.id}/cost`,
        ...
        { headers: { Authorization: `Bearer ${token}` } });
      toast.success('Cost updated successfully'); setActionModalOpen(false); fetchEnquiries();
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      await axios.put(`${import.meta.env.VITE_API_URL}/api/admin/enquiries/${assignTarget.id}/assign`,
        ...
        { headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` } });
      toast.success('Enquiry assigned successfully'); setAssignModalOpen(false); fetchEnquiries();
    } catch (err) { console.error(err); toast.error('Failed to assign enquiry'); }
  };

  const filterTabs = [
    { key: 'all', label: 'All' }, { key: 'new', label: 'New' }, { key: 'open', label: 'Open' },
    ...
    <div className="w-full">
      ...
      {!loading && !error && (
        <>
                  ...
                  filtered.map((enq, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex flex-wrap gap-2">
                          <button
                            onClick={e => { e.stopPropagation(); setSelectedId(enq.id); }}
          ...
          {/* Detail Drawer — fetches full details by ID */}
          {selectedId && (
            <DetailDrawer
