...
const InfoRow = ({ label, value }) => (
  <div className="flex justify-between items-start py-1.5 border-b border-white/[0.04] last:border-0">
    ...
    <span className="text-white/75 text-xs font-medium text-right max-w-[55%]">{value ?? '—'}</span>
  </div>
);
...
const DetailDrawer = ({ enquiryId, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex justify-end" onClick={onClose}>
      ...
      <div
        className="relative w-full max-w-lg h-full bg-gray-900 border-l border-white/[0.07] flex flex-col shadow-2xl"
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.06] flex-shrink-0">
          <div className="min-w-0">
            {loading ? (
              ...
              <>
                <p className="text-white/30 text-xs font-mono">{enq?.enquiryId}</p>
                <h3 className="text-white font-semibold text-sm mt-0.5 truncate">{enq?.description}</h3>
              </>
            )}
          </div>
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-lg bg-white/5 hover:bg-white/10 flex items-center justify-center text-white/40 hover:text-white/70 transition-all ml-3 flex-shrink-0"
          >
          ...
          {tabs.map(t => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap transition-all ${
                activeTab === t.key ? 'bg-indigo-500 text-white' : 'text-white/35 hover:text-white/60 hover:bg-white/[0.04]'
              }`}
            >
              {t.label}
            </button>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {loading ? (
            ...
            <p className="text-white/30 text-sm text-center mt-10">Failed to load details</p>
          ) : (
            <>
              {activeTab === 'overview' && (
                <div className="space-y-5">
                  <Card>
                    ...
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div>
                        <p className="text-white font-semibold text-sm">{enq.userId?.firstName} {enq.userId?.lastName}</p>
                  ...
                  <Card>
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <span className="text-white/25 text-xs py-1">{new Date(enq.createdAt).toLocaleDateString()}</span>
                    </div>
                  </Card>
                  {enq.sellingDetails && (
                    <Card>
              ...
              {activeTab === 'exterior' && car?.exterior_tyres && (
                <div className="space-y-4">
                  ...
                  <Card>
                    ...
                    <PartRow label="Bonnet / Hood" data={car.exterior_tyres.bonnet_hood} /><PartRow label="Roof" data={car.exterior_tyres.roof} /><PartRow label="Dicky / Boot Door" data={car.exterior_tyres.dicky_boot_door} /><PartRow label="Boot Floor" data={car.exterior_tyres.boot_floor} /><PartRow label="Running Border LHS" data={car.exterior_tyres.running_border?.lhs} /><PartRow label="Running Border RHS" data={car.exterior_tyres.running_border?.rhs} /><PartRow label="ORVM LHS" data={car.exterior_tyres.orvm?.lhs} /><PartRow label="ORVM RHS" data={car.exterior_tyres.orvm?.rhs} /><PartRow label="Alloy Wheel" data={car.exterior_tyres.alloy_wheel} /><PartRow label="Apron" data={car.exterior_tyres.apron} /><PartRow label="Cowl Top" data={car.exterior_tyres.cowl_top} /><PartRow label="Firewall" data={car.exterior_tyres.firewall} /><PartRow label="Head Light Support" data={car.exterior_tyres.head_light_support} /><PartRow label="Lower Cross Member" data={car.exterior_tyres.lower_cross_member} /><PartRow label="Upper Cross Member" data={car.exterior_tyres.upper_cross_member} /><PartRow label="Radiator Support" data={car.exterior_tyres.radiator_support} />
                  </Card>
                  {car.exterior_tyres.comments && <Card><SectionLabel>Exterior Comments</SectionLabel><p className="text-white/60 text-sm">{car.exterior_tyres.comments}</p></Card>}
                </div>
              )}
                ...
                <div className="space-y-4">
                  <Card>
                    ...
                    <div className="grid grid-cols-2 gap-x-4">
                      {[
                        ['Electrical', car.electricals_interior.electrical],['ABS', car.electricals_interior.abs?.status],['Airbag', car.electricals_interior.airbag_feature],['Parking Sensor', car.electricals_interior.parking_sensor],['Reverse Camera', car.electricals_interior.reverse_camera],['Power Windows', car.electricals_interior.power_windows],['Rear Defogger', car.electricals_interior.rear_defogger],['Music System', car.electricals_interior.music_system?.status],['Navigation Chip', car.electricals_interior.navigation_chip],['Sunroof', car.electricals_interior.sunroof],['Door Trim', car.electricals_interior.door_trim?.status],['Roof Lining', car.electricals_interior.roof_lining?.status],['Fabric Seat', car.electricals_interior.fabric_seat],['Leather Seat', car.electricals_interior.leather_seat?.status],['Steering Audio', car.electricals_interior.steering_mounted_audio_control],
                      ].map(([label, value]) => (
                  ...
                  {car.electricals_interior.abs && <Card><SectionLabel>ABS</SectionLabel><InfoRow label="Status" value={car.electricals_interior.abs.status} /><InfoRow label="Warning Light Glowing" value={car.electricals_interior.abs.warning_light_glowing ? 'Yes' : 'No'} /></Card>}
                  {car.electricals_interior.remote_key && <Card><SectionLabel>Remote Key</SectionLabel><InfoRow label="Available" value={car.electricals_interior.remote_key.available ? 'Yes' : 'No'} />{car.electricals_interior.remote_key.notes && <InfoRow label="Notes" value={car.electricals_interior.remote_key.notes} />}</Card>}
                  <Card><SectionLabel>Counts</SectionLabel><InfoRow label="No. of Airbags" value={car.electricals_interior.no_of_airbags} /><InfoRow label="No. of Power Windows" value={car.electricals_interior.no_of_power_windows} /></Card>
                  ...
                  {car.electricals_interior.comments && <Card><SectionLabel>Interior Comments</SectionLabel><p className="text-white/60 text-sm">{car.electricals_interior.comments}</p></Card>}
                </div>
              )}
                ...
                return (
                  <div className="space-y-4">
                    <Card>
                      ...
                      <InfoRow label="Status" value={et.engine?.status} /><InfoRow label="MIL Light Glowing" value={et.engine?.mil_light_glowing ? 'Yes' : 'No'} /><InfoRow label="Air Filter Box Damaged" value={et.engine?.air_filter_box_damaged ? 'Yes' : 'No'} /><InfoRow label="Electrical Wiring Damaged" value={et.engine?.electrical_wiring_damaged ? 'Yes' : 'No'} />
                      {et.engine?.conditions?.length > 0 && <div className="flex gap-1 flex-wrap mt-1">{et.engine.conditions.map((c, i) => <span key={i} className="text-[10px] px-1.5 py-0.5 rounded bg-rose-500/10 text-rose-400">{c}</span>)}</div>}
                    </Card>
                    <Card><SectionLabel>Battery</SectionLabel><InfoRow label="Status" value={et.battery?.status} /><InfoRow label="Acid Leakage" value={et.battery?.acid_leakage ? 'Yes' : 'No'} /></Card>
                    <Card><SectionLabel>Engine Oil & Coolant</SectionLabel><InfoRow label="Oil Status" value={et.engine_oil?.status} /><InfoRow label="Leakage from Tappet Cover" value={et.engine_oil?.leakage_from_tappet_cover ? 'Yes' : 'No'} /><InfoRow label="Coolant Status" value={et.coolant?.status} /><InfoRow label="Coolant Dirty" value={et.coolant?.dirty ? 'Yes' : 'No'} /><InfoRow label="Coolant Level Low" value={et.coolant?.level_low ? 'Yes' : 'No'} /></Card>
                    <Card><SectionLabel>Transmission</SectionLabel><InfoRow label="Clutch Status" value={et.clutch?.status} /><InfoRow label="Clutch Hard" value={et.clutch?.hard ? 'Yes' : 'No'} /><InfoRow label="Clutch Bearing Noise" value={et.clutch?.bearing_noise ? 'Yes' : 'No'} /><InfoRow label="Burning Smell" value={et.clutch?.burning_smell ? 'Yes' : 'No'} /><InfoRow label="Gear Shifting Status" value={et.gear_shifting?.status} /><InfoRow label="Gear Hard" value={et.gear_shifting?.hard ? 'Yes' : 'No'} /><InfoRow label="Not Engaging" value={et.gear_shifting?.not_engaging ? 'Yes' : 'No'} /><InfoRow label="Gearbox Oil Leakage" value={et.gear_shifting?.gearbox_oil_leakage ? 'Yes' : 'No'} /><InfoRow label="Front Drive Axle Noise" value={et.gear_shifting?.front_drive_axle_noise ? 'Yes' : 'No'} /></Card>
                    <Card><SectionLabel>Other Engine Parts</SectionLabel><InfoRow label="Engine Mounting" value={et.engine_mounting?.status} /><InfoRow label="Excess Vibration" value={et.engine_mounting?.excess_vibration ? 'Yes' : 'No'} /><InfoRow label="Engine Sound" value={et.engine_sound?.status} />{et.engine_sound?.notes && <InfoRow label="Sound Notes" value={et.engine_sound.notes} />}<InfoRow label="Exhaust / Smoke" value={et.exhaust_smoke?.status} /><InfoRow label="Silencer Assembly Damaged" value={et.exhaust_smoke?.silencer_assembly_damaged ? 'Yes' : 'No'} /><InfoRow label="Turbo Charger" value={et.turbo_charger?.status} /><InfoRow label="Turbo Whistling Noise" value={et.turbo_charger?.whistling_noise ? 'Yes' : 'No'} /><InfoRow label="Fuel Injector" value={et.fuel_injector?.status} /><InfoRow label="Injector Noise" value={et.fuel_injector?.noise ? 'Yes' : 'No'} /><InfoRow label="Sump Damaged" value={et.sump?.damaged ? 'Yes' : 'No'} /><InfoRow label="Sump Leakage" value={et.sump?.leakage ? 'Yes' : 'No'} /><InfoRow label="Radiator Fan Motor" value={et.radiator_fan_motor?.status} /><InfoRow label="Fan Motor Noise" value={et.radiator_fan_motor?.noise ? 'Yes' : 'No'} /></Card>
                    {car.steering_suspension_brakes && (
                      <Card>
                        <SectionLabel>Steering, Suspension & Brakes</SectionLabel>
                        <InfoRow label="Steering Status" value={car.steering_suspension_brakes.steering?.status} /><InfoRow label="Steering Hard" value={car.steering_suspension_brakes.steering?.hard ? 'Yes' : 'No'} /><InfoRow label="Steering Abnormal Noise" value={car.steering_suspension_brakes.steering?.abnormal_noise ? 'Yes' : 'No'} /><InfoRow label="Suspension Status" value={car.steering_suspension_brakes.suspension?.status} /><InfoRow label="Suspension Abnormal Noise" value={car.steering_suspension_brakes.suspension?.abnormal_noise ? 'Yes' : 'No'} /><InfoRow label="Brake Status" value={car.steering_suspension_brakes.brake?.status} /><InfoRow label="Brakes Noisy" value={car.steering_suspension_brakes.brake?.noisy ? 'Yes' : 'No'} />{car.steering_suspension_brakes.comments && <InfoRow label="Comments" value={car.steering_suspension_brakes.comments} />}
                      </Card>
                    )}
                    {et.towing_recommended && (
                      <div className="rounded-xl bg-rose-500/10 border border-rose-500/20 px-4 py-3 flex items-center gap-2">
                        <svg className="w-4 h-4 text-rose-400 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
                        <span className="text-rose-400 text-sm font-semibold">Towing Recommended</span>
                      </div>
                    )}
                    {et.comments && <Card><SectionLabel>Engine Comments</SectionLabel><p className="text-white/60 text-sm">{et.comments}</p></Card>}
                  </div>
                );
              })()}
              ...
              {activeTab === 'journey' && (
                <div className="space-y-4">
                  <Card><SectionLabel>Customer Journey</SectionLabel><InfoRow label="Current Step" value={enq.customerJourney?.currentStep} /></Card>
                      ...
                      <div className="relative pl-4">
                        ...
                        {enq.customerJourney.timeline.map((step, i) => (
                          <div key={i} className="relative mb-4 last:mb-0">
                            ...
                            <p className="text-white/70 text-sm font-semibold capitalize">{step.step}</p>
                  ...
                  <Card>
                    ...
                    <InfoRow label="Enquiry ID" value={enq.enquiryId} /><InfoRow label="Type" value={enq.enquiryType} /><InfoRow label="Severity" value={enq.severity} /><InfoRow label="Assigned To" value={enq.assignedTo ?? 'Unassigned'} /><InfoRow label="Schedule Date" value={enq.scheduleDate ? new Date(enq.scheduleDate).toLocaleDateString() : '—'} /><InfoRow label="Schedule Time" value={enq.scheduleTime || '—'} /><InfoRow label="Inspection Type" value={enq.inspectionType || '—'} /><InfoRow label="Created At" value={new Date(enq.createdAt).toLocaleString()} /><InfoRow label="Updated At" value={new Date(enq.updatedAt).toLocaleString()} />
                  </Card>
                </div>
              )}
            </>
  ...
  return (
    <div className="px-5 py-3 border-t border-white/[0.05] bg-white/[0.01] flex flex-col sm:flex-row items-center justify-between gap-3">
      ...
      <span className="text-white/25 text-xs">
        Showing <span className="text-white/40 font-medium">{from}–{to}</span> of <span className="text-white/40 font-medium">{total}</span> enquiries
      </span>
      ...
      <div className="flex items-center gap-1">
        ...
        <button
          ...
          className="w-8 h-8 rounded-lg bg-white/[0.04] border border-white/[0.06] flex items-center justify-center text-white/40 hover:text-white/70 hover:bg-white/[0.08] disabled:opacity-30 disabled:cursor-not-allowed transition-all"
        >
        ...
        {getPageNumbers().map((p, i) =>
          p === '...' ? (
            <span key={`dot-${i}`} className="w-8 h-8 flex items-center justify-center text-white/20 text-xs">…</span>
          ) : (
            <button
              key={p}
              ...
              className={`w-8 h-8 rounded-lg text-xs font-semibold border transition-all ${
                p === currentPage
                  ...
                  : 'bg-white/[0.04] border-white/[0.06] text-white/40 hover:bg-white/[0.08] hover:text-white/70'
              }`}
            >
              {p}
            </button>
...
const Enquiries = () => {
  ...
  useEffect(() => {
    fetchEnquiries(1);
  }, [filterStatus, limit]);  // search handled by explicit button / debounce below
  ...
  useEffect(() => {
    const fetchTechnicians = async () => {
      try {
        ...
        setTechnicians(response.data.data.technicians.map(t => ({ id: t._id, name: `${t.firstName} ${t.lastName}` })));
      } catch (err) { console.error('Failed to fetch technicians', err); }
    };
    fetchTechnicians();
  }, []);

  // ── Counts from pagination total (server-side) ───────────────────
  // For tab counts we use what's currently loaded; for "all" we use pagination.total
  const counts = {
  ...
  // When server-side filtering, no client-side filter needed
  const filtered = enquiries;

  // ── Handlers ─────────────────────────────────────────────────────
  const openAssignModal = (enquiry) => { setAssignTarget(enquiry); setSelectedTechId(''); setAssignPriority('medium'); setAssignModalOpen(true); };
  const openNoteModal   = (enquiry) => { setAssignTarget(enquiry); setNoteText(''); setActionType('note'); setActionModalOpen(true); };
  const openCostModal   = (enquiry) => { setAssignTarget(enquiry); setEstimatedCost(enquiry.estimatedCost || ''); setActualCost(enquiry.actualCost || ''); setActionType('cost'); setActionModalOpen(true); };
  ...
  const handleAddNote = async () => {
    ...
    try {
      ...
      await axios.post(`${import.meta.env.VITE_API_URL}/api/admin/enquiries/${assignTarget.id}/add-note`,
        { text: noteText }, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleUpdateCost = async () => {
    try {
      ...
      setActionModalOpen(false);
      fetchEnquiries(currentPage);
    } catch (err) { toast.error(err.response?.data?.message || 'Failed to update cost'); }
    finally { setSubmitting(false); }
  };
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      fetchEnquiries(currentPage);
    } catch (err) { console.error(err); toast.error('Failed to assign enquiry'); }
  };
      ...
      {!error && (
        <>
          ...
          <div className="grid grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
            <StatCard label="Total Enquiries" value={pagination?.total ?? counts.all} sub="All time" accent="bg-indigo-500"
                ...
                {filterTabs.map(tab => (
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all duration-200 whitespace-nowrap ${
                      filterStatus === tab.key ? 'bg-indigo-500 text-white shadow-lg shadow-indigo-500/30' : 'text-white/35 hover:text-white/60'
                    }`}
                  >
                    {tab.label}
              ...
              <div className="min-w-[640px]">
                ...
                {loading && (
                  <div className="divide-y divide-white/[0.04]">
                    {[...Array(limit)].map((_, i) => (
                      <div key={i} className="grid grid-cols-[2fr_2fr_1fr_1fr_160px] gap-4 px-5 py-4 items-center animate-pulse">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex flex-wrap gap-2" onClick={e => e.stopPropagation()}>
                        <button onClick={() => setSelectedId(enq.id)} className="p-1.5 rounded-lg bg-indigo-500/10 text-indigo-400 hover:bg-indigo-500/20 transition-all" title="View Details">
                        ...
                        <button onClick={() => openAssignModal(enq)} className="p-1.5 rounded-lg bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 transition-all" title="Assign Technician">
                        ...
                        <button onClick={() => openNoteModal(enq)} className="p-1.5 rounded-lg bg-amber-500/10 text-amber-400 hover:bg-amber-500/20 transition-all" title="Add Note">
                        ...
                        <button onClick={() => openCostModal(enq)} className="p-1.5 rounded-lg bg-blue-500/10 text-blue-400 hover:bg-blue-500/20 transition-all" title="Set Cost">
          ...
          {actionModalOpen && assignTarget && (
            <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={() => setActionModalOpen(false)}>
              <div className="w-full max-w-md rounded-2xl bg-gray-900 border border-white/[0.08] shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-6">
                  <div className="flex items-center justify-between mb-6">
                    <h3 className="text-white text-lg font-bold">{actionType === 'note' ? 'Add Note' : 'Set Margine'}</h3>
                  ...
                  {actionType === 'note' ? (
                    <div className="space-y-4">
                      ...
                      <button onClick={handleAddNote} disabled={submitting || !noteText.trim()}
                        className="w-full py-3 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 text-white font-bold text-sm disabled:opacity-50">
                    ...
                    <div className="space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-[10px] font-bold text-white/30 uppercase tracking-widest mb-2">Customer Offer (₹)</label>
                        ...
                        <div>
                          <label className="block text-[10px] font-bold text-white/30 uppercase tracking-widest mb-2">Highest Bid (₹)</label>
                          <input type="number" value={actualCost} onChange={e => setActualCost(e.target.value)} placeholder="0"
                            className="w-full bg-white/[0.03] border border-white/10 rounded-xl px-4 py-3 text-white text-sm outline-none focus:border-blue-500/50 transition-all" />
                        </div>
                      </div>
                      <button onClick={handleUpdateCost} disabled={submitting}
                        className="w-full py-3 rounded-xl bg-gradient-to-r from-blue-500 to-indigo-500 text-white font-bold text-sm disabled:opacity-50">
              ...
              <div className="w-full max-w-md rounded-2xl bg-gray-900 border border-white/[0.08] shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
                <div className="bg-gradient-to-r from-indigo-600/20 to-violet-600/10 border-b border-white/[0.06] px-6 py-5">
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={() => setAssignModalOpen(false)} className="w-8 h-8 rounded-lg bg-white/5 hover:bg-white/10 flex items-center justify-center text-white/40 hover:text-white/70 transition-all">
                ...
                <div className="p-6 space-y-5">
                  <div>
                    ...
                    <select value={selectedTechId} onChange={e => setSelectedTechId(e.target.value)}
                      className="w-full bg-gray-800 border border-white/10 rounded-xl px-4 py-3 text-white text-sm outline-none focus:border-indigo-500/60 transition-all appearance-none cursor-pointer"
                      style={{ colorScheme: 'dark' }}>
                      <option value="" disabled className="bg-gray-800 text-white/50">Choose a technician...</option>
                      {technicians.map(tech => (
                        <option key={tech.id} value={tech.id} className="bg-gray-800 text-white">{tech.name}</option>
                      ))}
                    </select>
                  </div>
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {[
                        ...
                        <button key={opt.value} onClick={() => setAssignPriority(opt.value)}
                          className={`py-2.5 rounded-xl border text-xs font-bold transition-all ${assignPriority === opt.value ? `${opt.activeBg} ${opt.border} ${opt.color}` : 'bg-white/[0.03] border-white/[0.07] text-white/30 hover:text-white/50'}`}>
                          {opt.label}
                        </button>
                      ))}
                    </div>
                ...
                <div className="px-6 pb-6 flex gap-3">
                  <button onClick={() => setAssignModalOpen(false)} className="flex-1 py-2.5 rounded-xl bg-white/[0.06] text-white/60 text-sm font-semibold hover:bg-white/[0.10] transition-all">Cancel</button>
                  <button onClick={handleAssign} disabled={!selectedTechId}
                    className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-indigo-500 to-violet-500 text-white text-sm font-bold disabled:opacity-50 shadow-lg shadow-indigo-500/25">
                    Assign Now →
                  </button>
                </div>
              </div>
