...
const roleConfig = {
  ...
  user:          { label: 'User',        bg: 'bg-white/[0.05]',  text: 'text-white/40',   border: 'border-white/[0.08]'  },
};
...
const TableView = ({ users, onUserClick }) => (
  <div className="overflow-x-auto rounded-2xl border border-white/[0.06] bg-gray-900">
    <table className="w-full min-w-[640px]">
      <thead>
        <tr className="border-b border-white/[0.06] bg-white/[0.02]">
          {['User', 'Contact', 'Role', 'Status', 'Joined', ''].map((h, i) => (
            <th key={i} className="px-5 py-3 text-left text-[10px] font-semibold text-white/35 uppercase tracking-widest">
              {h}
            </th>
          ))}
        </tr>
            ...
            <td className="px-5 py-3.5">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[11px] text-white/35 font-mono truncate">{user.id.slice(0, 14)}…</p>
                </div>
              </div>
            </td>
            <td className="px-5 py-3.5">
              <p className="text-sm text-white/65 truncate max-w-[180px]">{user.email}</p>
              <p className="text-[11px] text-white/35">{user.phone}</p>
            </td>
            <td className="px-5 py-3.5"><RoleBadge role={user.role} /></td>
            <td className="px-5 py-3.5"><StatusBadge status={user.status} /></td>
            <td className="px-5 py-3.5 text-xs text-white/40">{user.joined}</td>
            <td className="px-5 py-3.5">
...
const GridView = ({ users, onUserClick }) => (
  <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-4">
    {users.length === 0 && (
      <div className="col-span-full py-10 text-center text-sm text-white/30">No users found</div>
    )}
    {users.map(user => (
      <div
        key={user.id}
        onClick={() => onUserClick(user)}
        className="rounded-2xl border border-white/[0.06] bg-gray-900 p-5 hover:border-white/[0.12] hover:bg-white/[0.03] cursor-pointer transition-all duration-200 group"
      >
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            <Avatar initials={user.avatar} grad={user.avatarGrad} />
            <div className="min-w-0">
              <p className="font-semibold text-white/90 truncate text-sm leading-tight">{user.name}</p>
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-[11px] text-white/30">{user.joined}</span>
        </div>
        <div className="mt-3 pt-3 border-t border-white/[0.05] flex justify-between items-center">
          <span className="text-[11px] text-white/30">{user.phone}</span>
          <span className="text-xs font-semibold text-indigo-400 group-hover:text-indigo-300 transition-colors">
            View →
          </span>
        </div>
...
const UserModal = ({ user, onClose, onStatusChange }) => (
  ...
  >
    <div className="w-full max-w-md rounded-2xl bg-gray-900 border border-white/[0.10] shadow-2xl overflow-hidden">
      {/* Header */}
      <div className="relative px-6 pt-6 pb-5 border-b border-white/[0.06]">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 w-8 h-8 rounded-lg flex items-center justify-center text-white/40 hover:text-white hover:bg-white/[0.06] transition-all"
        >
          <IconX />
        </button>
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-white font-semibold text-lg leading-tight">{user.name}</h3>
      ...
      <div className="px-6 pb-6 flex gap-2">
        ...
        <button
          ...
          className="flex-1 py-2.5 bg-white/[0.05] hover:bg-white/[0.09] text-white/60 text-sm font-semibold rounded-xl transition-colors border border-white/[0.08]"
        >
          Set Inactive
        </button>
      </div>
  ...
  if (error) return (
    <div className="min-h-screen bg-gray-950 flex items-center justify-center p-8">
      <div className="text-center">
        ...
        <button onClick={fetchUsers} className="px-4 py-2 bg-indigo-600 rounded-xl text-sm font-semibold text-white">
          Retry
        </button>
      </div>
    </div>
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 py-8 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            ...
            <div className="relative flex-1 max-w-sm">
              ...
              <input
                ...
                className="w-full bg-white/[0.04] border border-white/[0.07] rounded-xl pl-10 pr-4 py-2.5 text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-indigo-500 focus:bg-white/[0.06] transition-all"
              />
            </div>
            ...
            <select
              ...
              className="bg-white/[0.04] border border-white/[0.07] rounded-xl px-4 py-2.5 text-sm text-white/70 focus:outline-none focus:border-indigo-500 transition-all cursor-pointer"
            >
              ...
              {[
                ...
                <button
                  ...
                  className={`w-9 h-9 rounded-lg flex items-center justify-center transition-all ${
                    view === v ? 'bg-indigo-600 text-white' : 'text-white/40 hover:text-white/70'
                  }`}
                >
                  {icon}
                </button>
        ...
        <p className="text-xs text-white/30">
          Showing <span className="text-white/50 font-medium">{filteredUsers.length}</span> of {users.length} users
        </p>
