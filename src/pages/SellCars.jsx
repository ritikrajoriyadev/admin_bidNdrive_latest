...
const StatCard = ({ label, value, icon, accentClass, change }) => (
  <div className="relative overflow-hidden rounded-2xl bg-gray-900 border border-white/[0.06] p-5 hover:border-white/[0.12] transition-all duration-300">
    ...
    <div className="flex items-center justify-between mb-4">
      <span className="w-9 h-9 rounded-xl bg-white/[0.06] flex items-center justify-center text-white/60">{icon}</span>
 ...
 const totalValue = Array.isArray(cars)
  ...
  const field = (label, key, type = 'text', placeholder = '') => (
    <div className="flex flex-col gap-2">
      ...
      <input
        ...
        className="px-4 py-2.5 rounded-xl bg-gray-800 border border-white/[0.06] text-white placeholder-white/20 focus:border-indigo-400/50 focus:outline-none transition-all"
      />
    </div>
  );
  ...
  const selectField = (label, key, options) => (
    <div className="flex flex-col gap-2">
      ...
      <select
        value={formData[key]}
        onChange={e => setFormData({ ...formData, [key]: e.target.value })}
        className="px-4 py-2.5 rounded-xl bg-gray-800 border border-white/[0.06] text-white focus:border-indigo-400/50 focus:outline-none transition-all"
      >
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Header */}
      <div className="flex flex-col gap-2">
        <h1 className="text-3xl font-bold text-white">Sell Cars</h1>
        <p className="text-white/40 text-sm">Manage your vehicle inventory and sales</p>
      </div>
      ...
      {showForm && (
        <div className="rounded-2xl bg-gray-900 border border-white/[0.06] p-6">
          <h2 className="text-xl font-bold text-white mb-6">{editingId ? 'Edit Car' : 'Add New Car'}</h2>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-8">
            ...
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-white/60 text-sm font-medium">Condition</label>
                  <select
                ...
                <div className="flex items-center gap-3 self-end pb-1">
                  ...
                  />
                  <label htmlFor="negotiable" className="text-white/60 text-sm font-medium">Price Negotiable</label>
                </div>
                <div className="flex flex-col gap-2 md:col-span-2">
                  ...
                  <textarea
                    ...
                    rows="3"
                    className="px-4 py-2.5 rounded-xl bg-gray-800 border border-white/[0.06] text-white placeholder-white/20 focus:border-indigo-400/50 focus:outline-none transition-all resize-none"
                  />
                </div>
              </div>
            </section>
            ...
            <div className="flex gap-3">
              ...
              <button
                type="button"
                onClick={resetForm}
                className="flex-1 px-4 py-2.5 rounded-xl bg-gray-800 border border-white/[0.06] text-white font-medium hover:bg-gray-700 transition-all duration-200"
              >
                Cancel
              </button>
            </div>
      ...
      <div className="rounded-2xl bg-gray-900 border border-white/[0.06] overflow-hidden">
        {loading ? (
          <div className="p-8 flex items-center justify-center text-white/40">Loading cars...</div>
        ) : filteredCars.length === 0 ? (
          <div className="p-8 flex items-center justify-center text-white/40">No cars found</div>
        ) : (
                  ...
                  return (
                    <tr key={id} className="hover:bg-white/[0.02] transition-colors duration-150">
                      <td className="px-6 py-4">
                        <p className="text-white font-semibold">{make} {model}</p>
                        <p className="text-white/40 text-xs">{desc}</p>
                      </td>
                      <td className="px-6 py-4 text-white">{year}</td>
                      <td className="px-6 py-4 text-white font-semibold">₹{Number(price).toLocaleString('en-IN')}</td>
                      <td className="px-6 py-4 text-white/60">{Number(miles).toLocaleString()} km</td>
                      <td className="px-6 py-4">
                        <div className="flex gap-2 flex-wrap">
                          <span className="text-[10px] px-2 py-1 rounded-full bg-white/[0.06] text-white/60">{fuel}</span>
                          <span className="text-[10px] px-2 py-1 rounded-full bg-white/[0.06] text-white/60">{trans}</span>
                        </div>
                      </td>
