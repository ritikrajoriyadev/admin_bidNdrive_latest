      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="space-y-4">
          ...
          <div className="flex flex-col gap-2">
            <label className="indigo-500/60 text-sm font-medium">Module *</label>
            <select
              required
          ...
          {/* Action */}
          <div className="flex flex-col gap-2">
            <label className="indigo-500/60 text-sm font-medium">Action *</label>
            <select
              required
...
const BulkPermissionForm = ({ onClose, onSave, saving }) => {
  ...
  return (
    <div className="rounded-2xl bg-white border border-white/[0.06] p-6 space-y-5">
      {/* Form Header */}
          ...
          <thead>
            <tr className="border-b border-white/[0.06] bg-gray-800/60">
              ...
              <th className="px-4 py-3 text-left w-44">
                <button
                  onClick={toggleAll}
                  className="text-xs indigo-500/40 hover:indigo-500/70 font-medium transition-colors"
                >
              ...
              {ACTIONS.map((act) => (
                <th key={act} className="px-3 py-3 text-center border-l border-white/[0.06]">
                  <div className="flex flex-col items-center gap-1.5">
                    <span className="text-xs font-semibold indigo-500/60 uppercase tracking-wider capitalize">{act}</span>
                    <button
                      onClick={() => toggleCol(act)}
                      className="text-[10px] indigo-500/30 hover:indigo-500/60 bg-white/5 hover:bg-white/10 rounded-md px-2 py-0.5 transition-all"
                    >
                      all
                    </button>
                  </div>
                ...
                <tr key={mod} className="hover:bg-white/[0.02] transition-colors group">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2.5">
                      ...
                      />
                      <span className="text-sm indigo-500 font-medium capitalize">{mod.replace('_', ' ')}</span>
                      <button
                        onClick={() => toggleRow(mod)}
                        className={`ml-auto text-[10px] rounded-md px-2 py-0.5 transition-all opacity-0 group-hover:opacity-100 ${rowAllOn
                          ? 'text-indigo-300 bg-indigo-500/15 hover:bg-indigo-500/25'
                          : 'indigo-500/40 bg-white/5 hover:bg-white/10 hover:indigo-500/70'
                          }`}
                      >
                  ...
                  {ACTIONS.map((act) => {
                    ...
                    return (
                      <td key={act} className="px-3 py-3 text-center border-l border-white/[0.04]">
                        ...
                        >
                          {checked && (
                            <svg className="w-3 h-3 indigo-500" viewBox="0 0 12 9" fill="none">
        ...
        <div className="flex flex-wrap gap-2">
          {selectedList.slice(0, 14).map(({ k, mod, act }) => (
            ...
            >
              ...
              <span className="indigo-500/60 capitalize">{act}</span>
              <button
                onClick={() => removePill(k)}
                className="ml-0.5 indigo-500/30 hover:indigo-500/70 transition-colors"
      ...
      <div className="flex flex-col gap-2">
        <label className="indigo-500/60 text-sm font-medium">
          Description
          <span className="indigo-500/30 font-normal ml-1">(applies to all created permissions)</span>
        </label>
        <textarea
          value={description}
      ...
      <div className="flex items-center gap-3 pt-1">
        ...
        <button
          ...
          className="ml-auto flex items-center gap-2 px-5 py-2.5 rounded-xl bg-indigo-500 indigo-500 text-sm font-medium hover:bg-indigo-600 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
        >
...
export default function Permissions() {
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Header */}
      <div className="flex flex-col gap-2">
        <h1 className="text-3xl font-bold indigo-500">Permission Management</h1>
        <p className="indigo-500/40 text-sm">Create and manage system permissions for fine-grained access control</p>
      </div>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <button
          onClick={openSingle}
          className={`flex items-center gap-2 px-4 py-2 rounded-xl border text-sm font-medium transition-all duration-200 ${formMode === 'single' && !editingPermission
            ? 'bg-white/10 border-white/20 indigo-500'
        ...
        <button
          onClick={openBulk}
          className={`flex items-center gap-2 px-4 py-2 rounded-xl border text-sm font-medium transition-all duration-200 ${formMode === 'bulk'
            ? 'bg-white/10 border-white/20 indigo-500'
      ...
      <div className="rounded-2xl bg-white border border-white/[0.06] overflow-hidden">
        {loading ? (
          <div className="p-8 flex items-center justify-center indigo-500/40">Loading permissions…</div>
        ) : filteredPermissions.length === 0 ? (
          <div className="p-8 flex items-center justify-center indigo-500/40">No permissions found</div>
        ) : (
                ...
                {filteredPermissions.map((perm) => {
                  ...
                  return (
                    <tr key={perm._id} className="hover:bg-white/[0.02] transition-colors duration-150">
                      ...
                      <td className="px-6 py-4">
                        <p className="indigo-500 font-semibold font-mono text-sm">{perm.action}</p>
                      </td>
                      <td className="px-6 py-4 indigo-500/60 text-sm max-w-xs truncate">
                        {perm.description || <span className="indigo-500/20 italic">No description</span>}
                      </td>
