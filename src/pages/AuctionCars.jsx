      ...
      {auction.expiryDate && (
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-amber-50 border border-amber-200">
          ...
          <span className="text-gray-500 text-xs">
            Expires: <span className="text-amber-600 font-semibold">{auction.expiryDate}</span>
          </span>
        </div>
      )}
      ...
      <div>
        ...
        {sortedInterests.length === 0 ? (
          <div className="rounded-xl border border-dashed border-gray-200 py-8 flex flex-col items-center gap-2">
            ...
            <p className="text-gray-300 text-sm">No active interests</p>
          </div>
        ) : (
          <div className="space-y-2">
            {sortedInterests.map((interest, idx) => {
              ...
              // biderId may be a populated object or a string id
              const bidder = (interest.biderId && typeof interest.biderId === 'object')
                ? interest.biderId
                : (interest.bider && typeof interest.bider === 'object')
                  ? interest.bider
                  : {};
              const firstName = bidder.firstName || interest.bidderName?.split(' ')[0] || '?';
              const lastName = bidder.lastName || interest.bidderName?.split(' ').slice(1).join(' ') || '';
              const email = bidder.email || interest.bidderEmail || '—';
              const phone = bidder.phone || interest.bidderPhone || null;
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <p className="text-gray-400 text-xs font-bold uppercase tracking-wider flex items-center gap-1.5">
              ...
              Previous Bidders ({previousInterests.length})
            </p>
            <span className="text-[10px] text-gray-300">Carried over from earlier round</span>
          </div>

          <div className="space-y-2">
            {previousInterests.map((interest, idx) => {
              // Support both flat fields (bidderName / bidderEmail / bidderPhone)
              // and a populated biderId object
              const bidderObj = (interest.biderId && typeof interest.biderId === 'object')
                ? interest.biderId : {};

              const name = interest.bidderName
                || (bidderObj.firstName ? `${bidderObj.firstName} ${bidderObj.lastName || ''}`.trim() : null)
                || 'Unknown Bidder';
              const email = interest.bidderEmail || bidderObj.email || '—';
              const phone = interest.bidderPhone || bidderObj.phone || null;
              ...
              const movedAt = interest.movedAt
                ? new Date(interest.movedAt).toLocaleString('en-IN', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })
                : '—';
              ...
              return (
                <div key={interest._id || `prev-${idx}`}
                  className="rounded-xl border border-gray-100 bg-slate-50 p-3">
                  <div className="flex items-center gap-3">
                    {/* Avatar */}
                    <div className={`w-9 h-9 rounded-full bg-gradient-to-br ${grad} flex items-center justify-center text-white text-xs font-bold flex-shrink-0 opacity-80`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-gray-300 text-[10px] mt-1 flex items-center gap-1">
                        <Calendar className="w-2.5 h-2.5" /> Moved at {movedAt}
                      </p>
                    </div>
              ...
              <table className="w-full whitespace-nowrap min-w-[960px]">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50">
                    {['Enquiry ID', 'Date', 'Vehicle', '  Year',
                      ...(isBNBMode ? ['Starting Price'] : []),
                      isBNBMode ? 'Top Interest / Count' : 'Top Bid / Bids',
                      'Fuel ', 'Remaining Time', 'Action'].map(h => (
                    ...
                    return (
                      <tr key={a.id} className="hover:bg-gray-50/70 transition-colors duration-150">
                        ...
                        <td className="px-4 py-3">
                          <p className="text-gray-500 text-xs capitalize">{[a.enquiryId] || ''} </p>
                        ...
                        {isBNBMode && (
                          <td className="px-4 py-3">
                            <p className="font-semibold text-indigo-600 text-sm">
                              ₹{a.startingPrice?.toLocaleString() || 0}
                            </p>
                          </td>
                        )}
                          ...
                          <button
                            onClick={() =>
                              setSelectedEnquiryId(
                                isBNBMode
                                  ? a.enquiryDocId?._id || a.enquiryId
                                  : a.enquiryDocId?._id || a.enquiryDocId
                              )
                            }
