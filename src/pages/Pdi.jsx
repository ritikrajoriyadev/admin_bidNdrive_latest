...
const STATUS_STYLES = {
  ...
  unknown:     { pill: 'bg-white/10 text-white/60' },
};
...
const AssignModal = ({
  ...
  return (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-lg rounded-3xl border border-white/10 bg-gray-900 shadow-2xl shadow-black/50 overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-white/10 bg-gray-950 px-6 py-4">
          <div>
            <h3 className="text-[15px] font-semibold text-white">Assign PDI to Technician</h3>
            <p className="text-[11px] text-white/40 mt-0.5">Select a technician to assign this PDI request</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="rounded-2xl bg-white/4 border border-white/5 p-4">
            <p className="text-[10px] uppercase tracking-[0.18em] text-white/35 mb-3">PDI Request Details</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            {techniciansLoading ? (
              <div className="flex items-center justify-center py-10">
                ...
                <span className="ml-2 text-sm text-white/40">Loading technicians…</span>
              </div>
            ) : technicians.length === 0 ? (
              <div className="rounded-2xl border border-white/8 bg-gray-950 p-4 text-center text-sm text-white/40">
                  ...
                  <label
                    ...
                    className={`flex items-center gap-3 rounded-2xl border p-3 cursor-pointer transition ${
                      selectedTech === tech.id
                        ...
                        : 'border-white/8 bg-gray-950 hover:border-white/20'
                    }`}
                  >
                    <input
                      type="radio"
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] text-white/40 truncate">{tech.email}</p>
                    </div>
                    <span
                      className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${
                        tech.isActive ? 'bg-emerald-400' : 'bg-white/20'
                      }`}
                    />
                  </label>
                ))}
            ...
            <div className="flex gap-3 pt-1">
              <button
                type="button"
                onClick={onClose}
                className="flex-1 rounded-2xl border border-white/10 bg-white/4 px-4 py-3 text-sm font-medium text-white/70 hover:bg-white/8 transition"
              >
                Cancel
              </button>
              <button
                ...
                className="flex-1 inline-flex items-center justify-center gap-2 rounded-2xl bg-emerald-600 px-4 py-3 text-sm font-semibold text-white hover:bg-emerald-500 disabled:opacity-50 disabled:cursor-not-allowed transition"
              >
                {assigning ? (
                  <>
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] p-4 md:p-8 space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-white">PDI Requests</h1>
        ...
        <button
          ...
          className="inline-flex items-center gap-2 self-start rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-white/70 hover:border-white/20 hover:bg-white/10 transition"
        >
          <RefreshCcw size={15} />
          Refresh
        </button>
      ...
      {!loading && !error && (
        <div className="flex flex-wrap gap-3">
          <StatCard label="Total" count={counts.total} colorClass="text-white" />
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        <div className="flex flex-1 items-center gap-3 rounded-2xl border border-white/10 bg-gray-900 px-4 py-3 shadow-sm">
        ...
        <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-gray-900 px-4 py-3">
          ...
          <select
            ...
            className="rounded-lg border border-white/10 bg-gray-950 px-3 py-1.5 text-sm text-white outline-none cursor-pointer"
          >
      ...
      {/* ── Table ── */}
      {loading ? (
        <div className="rounded-3xl border border-white/10 bg-gray-900 p-14 text-center text-white/40">
        ...
        <div className="overflow-hidden rounded-3xl border border-white/10 bg-gray-900 shadow-lg shadow-black/20">
          {/* Table head — hidden on mobile */}
          <div className="hidden md:grid md:grid-cols-[2fr_1fr_1.4fr_1fr_1.1fr_auto] gap-4 rounded-t-3xl border-b border-white/10 bg-gray-950 px-6 py-3.5 text-[10px] uppercase tracking-[0.18em] text-white/35">
            <div>Car Details</div>
            <div>Location</div>
            <div>Requester</div>
            <div>Status</div>
          ...
          {filteredPdi.length === 0 ? (
            <div className="p-10 text-center text-white/40 text-sm">No PDI requests found.</div>
          ) : (
            filteredPdi.map((item) => (
              ...
              >
                ...
                <div className="space-y-0.5">
                  <p className="font-semibold text-white text-sm">{item.carDetails?.make || '—'}</p>
                ...
                <div className="text-sm">
                  <p className="text-white">{item.location?.city || '—'}</p>
                  <p className="text-[11px] text-white/35">{item.location?.state}</p>
                </div>

                {/* Requester */}
                <div>
                  <p className="text-sm font-medium text-white">{item.name}</p>
                  <p className="text-[11px] text-white/35">{item.email}</p>
                  <p className="text-[11px] text-white/35">{item.phone}</p>
                </div>
                ...
                {/* Date */}
                <div>
                  <p className="text-[12px] text-white">{formatDate(item.createdAt)}</p>
                  <p className="text-[11px] text-white/35">{item.preferredTime}</p>
                </div>
                ...
                <div className="flex items-center gap-2 justify-start md:justify-end">
                  {/* <button
                    ...
                    className="inline-flex items-center gap-1.5 rounded-xl bg-indigo-600 px-3 py-2 text-[11px] font-semibold text-white hover:bg-indigo-500 transition"
                  >
                    <ArrowRight size={12} />
                    Update
                  </button> */}
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 rounded-xl bg-emerald-700 px-3 py-2 text-[11px] font-semibold text-white hover:bg-emerald-600 transition"
                  >
                    <Users size={12} />
                    Assign
                  </button>
      ...
      {/* ── Update status panel ── */}
      {selectedItem && (
        <div className="rounded-3xl border border-white/10 bg-gray-900 p-6 shadow-lg shadow-black/20 animate-in slide-in-from-bottom-4 duration-200">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between mb-6">
            <div>
              <p className="text-[10px] uppercase tracking-[0.18em] text-white/35 mb-1">
                Update status for
              </p>
              <h2 className="text-lg font-semibold text-white">{selectedItem.subject}</h2>
              <p className="text-[12px] text-white/40 mt-0.5">ID: {selectedItem.id}</p>
            </div>
            <button
              onClick={() => setSelectedItem(null)}
              className="self-start inline-flex items-center gap-1.5 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white/50 hover:bg-white/10 transition"
            >
              <X size={14} />
              Cancel
            </button>
          ...
          <form onSubmit={handleUpdateStatus} className="grid gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2">
              <label className="block text-[12px] font-medium text-white/60">Status</label>
              <select
                value={statusUpdate}
                onChange={(e) => setStatusUpdate(e.target.value)}
                className="w-full rounded-2xl border border-white/10 bg-gray-950 px-4 py-3 text-sm text-white outline-none cursor-pointer"
              >
                {VALID_STATUSES.map((s) => (
                  <option key={s} value={s}>{s.replace(/_/g, ' ')}</option>
                ))}
            ...
            {/* Admin note */}
            <div className="space-y-2">
              <label className="block text-[12px] font-medium text-white/60">Admin Response</label>
              <textarea
                value={adminResponse}
                onChange={(e) => setAdminResponse(e.target.value)}
                rows={4}
                placeholder="Add a note for this update…"
                className="w-full rounded-2xl border border-white/10 bg-gray-950 px-4 py-3 text-sm text-white outline-none resize-none"
              />
            </div>
            ...
            <div className="md:col-span-2 grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-3">
                ...
                <button
                  type="submit"
                  disabled={updating}
                  className="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-emerald-500 px-4 py-3 text-sm font-semibold text-white hover:bg-emerald-400 disabled:opacity-60 disabled:cursor-not-allowed transition"
                >
                  {updating ? (
                    <>
                      <Loader2 className="h-4 w-4 animate-spin" />
