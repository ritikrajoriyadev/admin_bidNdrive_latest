            ...
            <>
              ...
              {activeTab === 'exterior' && car?.exterior_tyres && (
                <div className="space-y-4">
                  ...
                  {car.exterior_tyres.comments && <Card><SectionLabel>Exterior Comments</SectionLabel><p className="indigo-500/60 text-sm">{car.exterior_tyres.comments}</p></Card>}
                </div>
              )}
              ...
              {activeTab === 'interior' && car?.electricals_interior && (
                <div className="space-y-4">
                  ...
                  {car.electricals_interior.comments && <Card><SectionLabel>Interior Comments</SectionLabel><p className="indigo-500/60 text-sm">{car.electricals_interior.comments}</p></Card>}
                </div>
              )}
              ...
              {activeTab === 'engine' && car?.engine_transmission && (() => {
                ...
                return (
                  <div className="space-y-4">
                    ...
                    {et.comments && <Card><SectionLabel>Engine Comments</SectionLabel><p className="indigo-500/60 text-sm">{et.comments}</p></Card>}
                  </div>
                );
              })()}
  ...
  return (
    <div className="px-5 py-3 border-t border-white/[0.05] bg-white/[0.01] flex flex-col sm:flex-row items-center justify-between gap-3">
      <span className="indigo-500/25 text-xs">
        Showing <span className="indigo-500/40 font-medium">{from}–{to}</span> of <span className="indigo-500/40 font-medium">{total}</span> enquiries
      </span>
...
const RA_Assigned = () => {
  ...
  const fetchEnquiries = useCallback(async (page = 1) => {
    try {
      ...
      // "new" tab on the UI sends "assign-to-team" to the API
      if (filterStatus === 'new') {
  ...
  // ── Counts ───────────────────────────────────────────────────────
  // "new" tab counts enquiries whose real status is "assign-to-team"
  const counts = {
  ...
  // Filter tabs — "New" tab filters by assign-to-team on the API side
  const filterTabs = [
    { key: 'all', label: 'All' },
    { key: 'new', label: 'New' },   // maps to assign-to-team
    { key: 'open', label: 'Open' },
      ...
      {!error && (
        <>
          ...
          <div className="grid grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
            ...
            <StatCard label="New" value={counts.new} sub="Assign to team" accent="bg-cyan-500"
            ...
            <StatCard label="Open" value={counts.open} sub="Active" accent="bg-violet-500"
            ...
            <StatCard label="Resolved" value={counts.resolved} sub="Completed" accent="bg-emerald-500"
              ...
              <div className="flex items-center gap-1 bg-white/[0.04] rounded-xl p-1 border border-white/[0.06] w-max">
                {filterTabs.map(tab => (
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all duration-200 whitespace-nowrap ${filterStatus === tab.key ? 'bg-indigo-500 indigo-500 shadow-lg shadow-indigo-500/30' : 'indigo-500/35 hover:indigo-500/60'
                      }`}
                  >
                    {tab.label}
                ...
                {!loading && filtered.map((enq, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div>
                        {/* Status badge shows "New" for assign-to-team */}
                        <StatusDot status={enq.status} />
                      </div>
          ...
          {actionModalOpen && assignTarget && (
            <div className="fixed inset-0 z-[300] flex items-center justify-center indigo-500/70 backdrop-blur-sm p-4" onClick={() => setActionModalOpen(false)}>
              <div className="w-full max-w-md rounded-2xl bg-white border border-white/[0.08] shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-6">
                  <div className="flex items-center justify-between mb-6">
                    <h3 className="indigo-500 text-lg font-bold">{actionType === 'note' ? 'Add Note' : 'Set Margine'}</h3>
                    <button onClick={() => setActionModalOpen(false)} className="indigo-500/20 hover:indigo-500 transition-colors">
                  ...
                  {actionType === 'note' ? (
                    ...
                    <div className="space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-[10px] font-bold indigo-500/30 uppercase tracking-widest mb-2">Customer Offer (₹)</label>
                        ...
                        <div>
                          <label className="block text-[10px] font-bold indigo-500/30 uppercase tracking-widest mb-2">Highest Bid (₹)</label>
          ...
          {assignModalOpen && assignTarget && (
            <div className="fixed inset-0 z-[300] flex items-center justify-center indigo-500/70 backdrop-blur-sm p-4" onClick={() => setAssignModalOpen(false)}>
              <div className="w-full max-w-md rounded-2xl bg-white border border-white/[0.08] shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
                <div className="bg-gradient-to-r from-indigo-600/20 to-violet-600/10 border-b border-white/[0.06] px-6 py-5">
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={() => setAssignModalOpen(false)} className="w-8 h-8 rounded-lg bg-white/5 hover:bg-white/10 flex items-center justify-center indigo-500/40 hover:indigo-500/70 transition-all">
                ...
                <div className="p-6 space-y-5">
                  ...
                  <div>
                    <label className="block text-xs font-bold indigo-500/40 uppercase tracking-widest mb-2">Priority Level</label>
                    <div className="grid grid-cols-3 gap-2">
                      {[
                        ...
                        <button key={opt.value} onClick={() => setAssignPriority(opt.value)}
                          className={`py-2.5 rounded-xl border text-xs font-bold transition-all ${assignPriority === opt.value ? `${opt.activeBg} ${opt.border} ${opt.color}` : 'bg-white/[0.03] indigo-500 indigo-500/30 hover:indigo-500/50'}`}>
                          {opt.label}
                        </button>
                      ))}
                    </div>
                ...
                <div className="px-6 pb-6 flex gap-3">
                  <button onClick={() => setAssignModalOpen(false)} className="flex-1 py-2.5 rounded-xl bg-white/[0.06] indigo-500/60 text-sm font-semibold hover:bg-white/[0.10] transition-all">Cancel</button>
