...
const StatCard = ({ label, value, icon, accent, change, positive }) => (
  <div className="relative overflow-hidden rounded-2xl bg-white border border-white/[0.06] p-5 hover:border-white/[0.12] transition-all duration-300 group">
    ...
    <div className="flex items-center justify-between mb-3">
      <span className={`w-9 h-9 rounded-xl flex items-center justify-center ${accent.replace('bg-', 'bg-')} bg-opacity-20 indigo-500`}>
        {icon}
      </span>
      {change && (
    ...
    <p className="indigo-500 text-3xl font-bold tracking-tighter">{value}</p>
    <p className="indigo-500/40 text-xs font-medium mt-1 tracking-widest uppercase">{label}</p>
  </div>
);
...
const Technicians = () => {
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="indigo-500/60">Loading technicians...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-white indigo-500 p-6">
      {/* Stats Cards */}
      ...
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 mb-6">
        <div className="flex flex-wrap items-center gap-2">
          <button
            ...
            className="px-4 py-2.5 bg-gradient-to-r from-indigo-500 to-violet-500 indigo-500 text-sm font-semibold rounded-2xl hover:from-indigo-400 hover:to-violet-400 transition-all shadow-lg shadow-indigo-500/30"
          >
            + Create Technician
          </button>
          <select
            ...
            className="bg-white/[0.04] border indigo-500 rounded-2xl px-4 py-3 text-sm outline-none focus:border-indigo-500 transition-all"
          >
        ...
        <div className="flex items-center gap-3 w-full lg:w-auto">
          <div className="relative flex-1 lg:w-80">
            <svg className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 indigo-500/40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            ...
            <input
              ...
              className="w-full bg-white/[0.04] border indigo-500 rounded-2xl pl-11 py-3 text-sm focus:border-indigo-500 focus:bg-white/[0.06] transition-all"
            />
          </div>
          ...
          <div className="flex bg-white/[0.04] rounded-2xl p-1 border border-white/[0.06]">
            {['table', 'grid'].map(v => (
              <button
                key={v}
                onClick={() => setView(v)}
                className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all ${view === v ? 'bg-indigo-600 indigo-500 shadow' : 'hover:bg-white/10 indigo-500/40'}`}
              >
                {v === 'table' ? '≡' : '▦'}
              </button>
            ))}
...
const TableView = ({ technicians, statusConfig, onClick }) => (
  <div className="rounded-2xl bg-white border border-white/[0.06] overflow-hidden">
    <div className="grid grid-cols-[2fr_2fr_1fr_1fr_80px] gap-4 px-6 py-4 border-b border-white/[0.05] bg-white/[0.02] indigo-500/40 text-xs font-semibold tracking-widest uppercase">
      <div>Name</div>
      <div>Contact</div>
      <div>Joined</div>
      <div>Status</div>
    ...
    {technicians.length === 0 ? (
      <div className="p-12 text-center indigo-500/40">No technicians found</div>
    ) : (
        ...
        return (
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-medium indigo-500/90">{tech.name}</p>
                <p className="text-xs indigo-500/40 font-mono">{tech.id}</p>
              </div>
            </div>

            <div className="text-sm indigo-500/70">
              <p>{tech.email}</p>
              <p className="indigo-500/50 text-xs">{tech.phone}</p>
            </div>

            <div className="text-sm indigo-500/60">{tech.joined}</div>
...
const GridView = ({ technicians, statusConfig, onClick }) => (
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
      ...
      return (
        ...
        >
          <div className="flex justify-between items-start mb-4">
            <div className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${tech.avatarGrad} flex items-center justify-center text-lg font-bold indigo-500`}>
              {tech.avatar}
            </div>
          ...
          <p className="indigo-500/50 text-sm mb-3 line-clamp-1">{tech.email}</p>

          <div className="flex items-center justify-between text-xs indigo-500/60">
    ...
    {technicians.length === 0 && (
      <div className="col-span-full py-12 text-center indigo-500/40">No technicians found</div>
    )}
  </div>
);
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center indigo-500/70 backdrop-blur-sm p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="p-6">
          <button
            onClick={onClose}
            className="absolute top-5 right-5 w-8 h-8 flex items-center justify-center indigo-500/40 hover:indigo-500 transition-colors"
          >
            ✕
          </button>

          <div className="flex items-center gap-4 mb-6">
            <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${technician.avatarGrad} flex items-center justify-center text-2xl font-bold indigo-500`}>
              {technician.avatar}
            </div>
            <div>
              <h2 className="text-2xl font-semibold">{technician.name}</h2>
              <p className="indigo-500/50">{technician.email}</p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            ...
            <button
              ...
              className={`flex-1 py-3 rounded-2xl font-medium transition-all border bg-white/5 hover:bg-white/10 border-white/10 indigo-500/70`}
            >
              Mark Inactive
            </button>
          </div>
...
const CreateTechnicianModal = ({ formData, setFormData, onSubmit, onClose, creating }) => {
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center indigo-500/70 backdrop-blur-sm p-4" onClick={onClose}>
      <div
        className="w-full max-w-md bg-white rounded-3xl border border-white/[0.08] overflow-hidden"
        onClick={e => e.stopPropagation()}
      >
        ...
        <div className="p-6">
          ...
          <p className="indigo-500/50 text-sm mb-6">Add a new technician to your platform</p>
          ...
          <form onSubmit={onSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <input
                ...
                className="bg-white/[0.04] border indigo-500 rounded-lg px-3 py-2.5 text-sm indigo-500 placeholder-white/30 outline-none focus:border-indigo-500 focus:bg-white/[0.06] transition-all"
              />
              <input
                type="text"
            ...
            <input
              ...
              className="w-full bg-white/[0.04] border indigo-500 rounded-lg px-3 py-2.5 text-sm indigo-500 placeholder-white/30 outline-none focus:border-indigo-500 focus:bg-white/[0.06] transition-all"
            />
            ...
            <div className="flex gap-3 pt-4">
              <button
                type="submit"
                disabled={creating}
                className="flex-1 py-3 bg-gradient-to-r from-indigo-500 to-violet-500 hover:from-indigo-400 hover:to-violet-400 disabled:opacity-50 disabled:cursor-not-allowed indigo-500 font-semibold rounded-lg transition-all"
              >
